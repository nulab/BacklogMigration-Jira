...
private [writer] class IssueCategoriesWrites @Inject()() extends Writes[Seq[Component], Seq[BacklogIssueCategory]] {
  ...
  private [this] def toBacklog(category: Component) =
    BacklogIssueCategory(optId = Some(category.id), name = category.name, delete = false)
