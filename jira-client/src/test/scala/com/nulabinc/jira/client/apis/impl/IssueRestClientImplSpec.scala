...
class IssueRestClientImplSpec extends Specification with Mockito {
  ...
  "should get an Issue" >> {
    ...
    val fields = IssueField(Some("example bug report"))
    val expect = Issue(10002, "EX-1", fields)
  ...
  "should get issues belonging to the project" >> {
    ...
    val client = new IssueRestClientImpl(httpClient)
    val expect = Seq(
      Issue(10002, "EX-1", IssueField(Some("example bug report"))),
      Issue(10003, "EX-2", IssueField(Some("example bug report 2")))
    )
