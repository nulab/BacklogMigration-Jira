...
class JiraClientCommentService @Inject()(jira: JiraRestClient) extends CommentService {
  ...
  override def issueComments(issue: Issue): Seq[Comment] = fetch(issue, 0, 100, Seq.empty[Comment])

  private def fetch(issue: Issue, startAt: Long, maxResults: Long, comments: Seq[Comment]): Seq[Comment] =
