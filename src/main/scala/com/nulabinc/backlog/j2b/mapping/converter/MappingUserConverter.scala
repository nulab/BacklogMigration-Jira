...
class MappingUserConverter()(implicit val userWrites: UserWrites)
  ...
  override def convert(mappingCollectDatabase: MappingCollectDatabase, mappings: Seq[Mapping], user: String): String =
  ...
  private def mappingOfInfoName(mappingCollectDatabase: MappingCollectDatabase, userName: String): Mapping = {
    mappingCollectDatabase.findUser(userName) match {
      case Some(user) => Mapping(None, user.displayName, user.displayName)
