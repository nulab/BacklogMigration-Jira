...
case class ArrayFieldValue(values: Seq[FieldValue])
    extends FieldValue(values.map(_.value).mkString("\n"))
case class OptionFieldValue(v: IssueFieldOption)
    extends FieldValue(v.value.value)
