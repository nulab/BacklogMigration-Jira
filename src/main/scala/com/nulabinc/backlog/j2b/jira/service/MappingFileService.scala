...
trait MappingFileService {
  ...
  def createUserMappingFile(
      users: Set[User],
      backlogUsers: Seq[BacklogUser]
  ): MappingFile

  def createPriorityMappingFile(
      priorities: Seq[Priority],
      backlogPriorities: Seq[BacklogPriority]
  ): MappingFile

  def createStatusMappingFile(
      statuses: Seq[Status],
      backlogStatuses: BacklogStatuses
  ): MappingFile

  def createUserMappingFileFromJson(
      jiraUsersFilePath: Path,
      backlogUsers: Seq[BacklogUser]
  ): MappingFile

  def createPrioritiesMappingFileFromJson(
      jiraPrioritiesFilePath: Path,
      backlogPriorities: Seq[BacklogPriority]
  ): MappingFile

  def createStatusesMappingFileFromJson(
      jiraStatusesFilePath: Path,
      backlogStatuses: BacklogStatuses
  ): MappingFile
