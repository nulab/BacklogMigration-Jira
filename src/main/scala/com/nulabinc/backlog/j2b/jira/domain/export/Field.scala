...
object FieldType {
  ...
  case object FixVersions
      extends FieldType("fixVersions", BacklogFieldType.MultipleList)
  ...
  case object Versions
      extends FieldType("versions", BacklogFieldType.MultipleList)
  case object IssueLinks
      extends FieldType("issuelinks", BacklogFieldType.MultipleList)
  ...
  case object Components
      extends FieldType("components", BacklogFieldType.MultipleList)
  case object TimeTracking
      extends FieldType("timetracking", BacklogFieldType.Text)
  case object WorkLogs
      extends FieldType("worklog", BacklogFieldType.MultipleList)
  case object Option extends FieldType("option", BacklogFieldType.SingleList)
  case object OptionWithChild
      extends FieldType("option-with-child", BacklogFieldType.SingleList)
  case object SubTasks
      extends FieldType("subtasks", BacklogFieldType.MultipleList)
  ...
  case object CustomLabels
      extends FieldType("labels", BacklogFieldType.MultipleList)
  case object MultiSelect
      extends FieldType("multiselect", BacklogFieldType.MultipleList)
  case object SingleSelect
      extends FieldType("select", BacklogFieldType.SingleList)
  ...
  def apply(
    ...
    (schemaType, schemaSystem, schemaItems, schemaCustom) match {
      ...
      // Check customizable
      case (Some(String.value), None, None, Some(custom))
          if custom.contains(Radio.value) =>
        Radio
      case (Some(String.value), None, None, Some(custom))
          if custom.contains(Checkbox.value) =>
        Checkbox
      case (Some(String.value), None, None, Some(custom))
          if custom.contains(CustomLabels.value) =>
        CustomLabels
      case (Some(String.value), None, None, Some(custom))
          if custom.contains(MultiSelect.value) =>
        MultiSelect
      case (Some(String.value), None, None, Some(custom))
          if custom.contains(SingleSelect.value) =>
        SingleSelect
      case (Some(String.value), None, None, Some(custom))
          if custom.contains(TextArea.value) =>
        TextArea
      // Check single
