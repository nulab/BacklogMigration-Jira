...
object AppConfigValidator extends Logging {
  ...
  def apply(jiraRestClient: JiraRestClient, spaceService: SpaceService) =
    new AppConfigValidator(jiraRestClient, spaceService)
  ...
  def validateProjectKey(projectKey: String): ConfigValidateResult =
    if (projectKey.matches("""^[0-9A-Z_]+$""")) ConfigValidateSuccess
      ...
      ConfigValidateFailure(
        s"- ${Messages("cli.param.error.project_key", projectKey)}"
      )

  def validateConfigJira(
      jiraRestClient: JiraRestClient
    ...
    jiraRestClient.myself() match {
      ...
      case Left(error: HttpError) =>
        ...
        error.clientError match {
          case AuthenticateFailedError =>
            ConfigValidateFailure(
              s"- ${Messages("cli.param.error.auth", Messages("common.src"))}"
            )
          case unknown =>
            ConfigValidateFailure(
              s"- ${Messages("cli.param.error.client.unknown", unknown.message)}"
            )
        }
      case Left(e) =>
        logger.error(e.message, e)
        ConfigValidateFailure(
          s"- ${Messages("cli.param.error.disable.access.jira", Messages("common.src"))}"
        )
    }
  }

  def validateConfigBacklog(
      spaceService: SpaceService,
    ...
    try {
      ...
      case unknown: BacklogAPIException if unknown.getStatusCode == 404 =>
        ...
        ConfigValidateFailure(
          s"- ${Messages("cli.param.error.disable.host", Messages("common.dst"), config.url)}"
        )
      case e: Throwable =>
        logger.error(e.getMessage, e)
        ConfigValidateFailure(
          s"- ${Messages("cli.param.error.disable.access.backlog", Messages("common.dst"))}"
        )
    }
  }

  def validateJiraProject(
      jiraRestClient: JiraRestClient,
      config: JiraApiConfiguration
  ): ConfigValidateResult =
    jiraRestClient.projectAPI.project(config.projectKey) match {
      ...
      case Left(error) =>
        ...
        ConfigValidateFailure(
          s"- ${Messages("cli.param.error.disable.project", config.projectKey)}"
        )
    }
  ...
  def validateAuthBacklog(spaceService: SpaceService): ConfigValidateResult = {
    ...
    if (spaceService.hasAdmin()) {
      ...
      ConfigValidateFailure(s"- ${Messages("cli.param.error.auth.backlog")}")
  }
}
